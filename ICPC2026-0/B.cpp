...
ll makePalindrome(ll prefix, bool odd) {
    ll res = prefix;
    if (odd) prefix /= 10;
    while (prefix) {
...
ll countPalindromes(ll X){
    string s = to_string(X);
    int n = s.size();
    ll res = 0;

    for (int len = 1; len < n; len++) {
        res += 9 * pow(10, (len - 1) / 2);
    }

    ll prefix = stoll(s.substr(0, (n + 1) / 2));
    res += prefix - pow(10, (n - 1) / 2);

    if (makePalindrome(prefix, n % 2 == 1) <= X) {
        res++;
    }
    return res;
}
...
int main(){
    ...
    while (N--){
        cin >> L >> R;
        cout << countPalindromes(R) - countPalindromes(L - 1) << endl;
    }
    return 0;
}
